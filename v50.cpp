...
void searchInLevel(MyMap <string, int>:: iterator, MyVec<string> &, bool mode);
...
void searchInLevel(MyMap <string, int>:: iterator it, MyVec<string> &query, bool mode) {
    ...
    while(newWords){    // enquanto precisar de palavras
        ...
        switch (numWords) {
            case 0: // Caso tenha sido passado 0 palavras iniciais
                if (dic.lv1.size()) {
                    searchInLevel(it, query, mode);   // Busca e adiciona a melhor palavra para o contexto
