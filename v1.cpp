...
string formatLine(const string &line)
{
    ...
    {
        if (isAlpha(line[i]) or line[i] == '-')
        {                         // Caractere alfabético ou hífen
        ...
        else if (isEspecialCase(line[i]) and formatedLine.size() != 0 and (i < line.size() - 1))
        { // Se for um caractere do caso especial não sendo o último, na condição de que já haja letra na string de copia
...
bool identSentences(MyVec<MyVec<string>> &text, const string &lineFormated, bool needLine)
{
    ...
    if (!text.size())
    { // Se MyVector está vazio, adiciona o primeiro objeto newLine
    ...
    {
        if (needLine)
        { // Se precisa de uma nova linha, aloca em text e incrementa o indice
            MyVec<string> newLine;
            text.push_back(newLine);
        ...
        if (lineFormated[i] == '\n' or lineFormated[i] == ',')
        { // aloca a ultima palavra formada na linha atual
        ...
        else if (i == lineFormated.size() - 1)
        { // Indica que a frase inserida até o momento acabou, porém a senetença não, insere a palavra na linha
            if (word == "!")
                word = lineFormated[i];
            else
                word += lineFormated[i];
...
void populateDictionary(Dictionary &dic, const MyVec<MyVec<string>> &text)
{
    ...
    { // Percorre todas as linhas e colunas de text
        for (int j = 0; j < text[i].size(); j++)
        {
            ...
            if (j + 2 < text.size()) // Se existir uma palavra à frente cria uma relação entre essa e "frase" já formada, incrementa a frequência
...
void search(const MyVec<string> &query, Dictionary &dic)
{
    ...
    if (rows > 0)
    { // Se o numero de linhas buscadas for maior que 0
